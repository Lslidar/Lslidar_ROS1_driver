...
/** @brief Get one lslidar packet. */
    int InputSocket::getPacket(lslidar_n301_driver::LslidarPacketPtr &pkt) {
        struct pollfd fds[1];
        fds[0].fd = sockfd_;
        ...
        if (!running.load()) {
            ROS_INFO("Shutting down getPacket gracefully.");
            return -1;  // 运行中断
        }
...
/** @brief Get one lslidar packet. */
    int InputPCAP::getPacket(lslidar_n301_driver::LslidarPacketPtr &pkt) {
