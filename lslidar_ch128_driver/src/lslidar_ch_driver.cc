...
bool LslidarChDriver::createRosIO() {
  ...
  // Output
  packet_pub = nh.advertise<lslidar_ch128_msgs::LslidarChPacket>(
        "lslidar_packet_ch128", 100);
  ...
  double packet_rate = 3571;
    ...
    if(dump_file !="")
    {
        ...
        difop_input_.reset(new lslidar_ch_driver::InputPCAP(pnh,difop_udp_port,packet_rate,dump_file));
    }else{
...
bool LslidarChDriver::polling()
{
    ...
    lslidar_ch128_msgs::LslidarChPacketPtr packet(
                new lslidar_ch128_msgs::LslidarChPacket());
    ...
    if (time_synchronization_ && (GPSCurrentTS > 0))
    {
      ...
      lslidar_ch128_msgs::LslidarChPacket pkt = *packet;
...
{
  lslidar_ch128_msgs::LslidarChPacketPtr difop_packet(
              new lslidar_ch128_msgs::LslidarChPacket());
  ...
  while (ros::ok())
  {
    ...
    if (rc == 0)
    {
      ...
      angle_1.bytes[0] = difop_packet->data[87];
      angle_1.bytes[1] = difop_packet->data[86];
      this->prism_angle[0] = angle_1.value * 0.01;

        angle_2.bytes[0] = difop_packet->data[89];
        angle_2.bytes[1] = difop_packet->data[88];
        this->prism_angle[1] = angle_2.value * 0.01;

        angle_3.bytes[0] = difop_packet->data[91];
        angle_3.bytes[1] = difop_packet->data[90];
        this->prism_angle[2] = angle_3.value * 0.01;

        angle_4.bytes[0] = difop_packet->data[93];
        angle_4.bytes[1] = difop_packet->data[92];
...
void LslidarChDriver::getFPGA_GPSTimeStamp(lslidar_ch128_msgs::LslidarChPacketPtr &packet)
{
