...
/** @brief destructor */
    ...
    int InputSocket::getPacket(lslidar_ch_msgs::LslidarChPacketPtr &packet,std::string packet_name) {
        ...
        while (flag == 1) {
            ...
            do {
                ...
                if (retval == 0 && packet_name == "msop_packet")            // poll() timeout?
                {
                    ROS_WARN("lslidar[%s] poll() timeout",packet_name.c_str());
                    return 1;
                }
...
/** @brief Get one lslidar packet. */
    int InputSocket::getPacket(lslidar_ch_msgs::LslidarChPacket *pkt, const double time_offset) {
        ...
        if (pkt->data[0] == 0xA5 && pkt->data[1] == 0xFF && pkt->data[2] == 0x00 && pkt->data[3] == 0x5A) {//difop
